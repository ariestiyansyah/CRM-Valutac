...
<div class="wojo basic button">
  <form action="https://www.moneybookers.com/app/payment.pl" method="post" class="xform" id="mb_form" name="mb_form">
    <input type="image" src="gateways/moneybookers/moneybookers_big.png" style="vertical-align:middle;border:0;width:160px;margin-right:10px" name="submit" title="Pay With Moneybookers" alt="" onclick="document.mb_form.submit();"/>
    ...
    <input type="hidden" name="return_url" value="<?php echo SITEURL;?>/account.php?do=billing" />
    <input type="hidden" name="cancel_url" value="<?php echo SITEURL;?>/account.php?do=billing" />
    ...
    <input type="hidden" name="custom" value="<?php echo $row2->id.'_'.$user->sesid;?>" />
    <input type="hidden" name="amount" value="<?php echo $amount;?>" />
    <input type="hidden" name="currency" value="<?php echo ($user->currency ? $user->currency : ($row->extra2 ? $row->extra2 : $core->currency));?>" />
    <input type="hidden" name="detail1_text" value="<?php echo cleanOut($row2->ptitle);?>" />
    <input type="hidden" name="detail1_description" value="Project Invoice:">
  </form>
</div>
...
<div class="wojo basic button">
  <form action="<?php echo $url;?>" class="xform" method="post" id="2co_form" name="2co_form">
    ...
    <input type="hidden" name="cart_order_id" value="INV<?php echo $row2->id;?>" />
    <input type="hidden" name="x_receipt_link_url" value="<?php echo SITEURL.'/gateways/'.$row->dir;?>/ipn.php" />
    <input type="hidden" value="1" name="c_prod">
    <input type="hidden" value="Invoice" name="c_name">
    <input type="hidden" value="Project Invoice - <?php echo cleanOut($row2->ptitle);?>" name="c_description">

(function ($) {
    $.forms = function (settings) {
        ...
        // Clone Checkbox Options
        $("body").on("click", "#btncAdd", function () {
            $("#btnDel").show();
            var checkFieldHtml = function () {
                var num = $('.clonedInput').length;
                var newNum = new Number(num + 1);
				var name = $('#container1 input:eq(0)').prop('name');
				var price = $('#container1 input:eq(1)').prop('name');
				field = '<div id="container' + newNum + '" class="two fields clonedInput">';
				field += '<div class="field"> ';
				field += '<label class="input">';
				field += '<input type="text" name="' + name + '">';
				...
				field += '</div>';
				field += '<div class="field">';
				field += '<div class="two fields">';
                field += '<div class="field">';
				field += '<label class="input"><i class="icon-prepend icon dollar"></i>';
				field += '<input type="text" name="' + price + '">';
				field += '</label>';
				field += '</div>';
                field += '<div class="field">';
				field += '<label class="checkbox">';

<?php
  ...
  class Estimator
  {
      ...
      const fTable = "estimator_fields";
	  const dTable = "estimator_data";
	  const lTable = "estimator_layout";
	  const cTable = "cart";
	  ...
	  public function sendForm()
	  {
		  ...
		  if ($fieldData) {
			  ...
			  $html = '<table width="100%" cellpadding="5" cellspacing="5" class="wojo basic table">';
						  ...
						  if (isset($_POST[$row->name])) {
							  ...
							  $html .= '<td style="text-align: left; border-bottom:1px dotted rgb(102, 102, 102)">';
									...
									if (is_array($_POST[$row->name])) {
										$res = array();
										$pricerow = explode('::', $row->price);
										foreach ($_POST[$row->name] as $i => $crow) {
											if ($crow != '') {
												array_push($res, sanitize($crow . ' (' . Registry::get("Core")->formatMoney($pricerow[$i]) . ')'));
											}
										}
										$datares = implode(', ', $res);
									}
			  ...
			  $html .= '<tr>
						  <td width="150" style="text-align: left; border-bottom:1px dotted rgb(102, 102, 102)"> ' . Lang::$word->FEST_ORDER_TOTAL . ':</td>
						  <td style="text-align: left; border-bottom:1px dotted rgb(102, 102, 102)">' . Registry::get("Core")->formatMoney($_POST['totalPrice']) . '</td>
						</tr>';
			  $html .= '</table>';
			  ...
			  if (empty(Filter::$msgs)) {
				  $data = array(
						  'form_id' => $form->id,
						  'sesid' => Registry::get("Users")->sesid,
				  ...
				  self::$db->delete(self::cTable, "sesid = '" . Registry::get("Users")->sesid . "'");
				  self::$db->insert(self::cTable, $data2);
				  
				  //print ($form->is_service) ? "CO" : "OK";
      ...
      public function getFields($fid = false)
      {
          ...
          $sql = "SELECT l.*, f.title, f.name, f.desc, f.msgerror, f.tooltip, f.type, f.attr, f.defval, f.required, f.multiple, f.other, f.price"  
		  . "\n FROM " . self::lTable . " l" 
      ...
      /**
       * Estimator::getFormbBySesid()
       * 
       * @return
       */
      public function getFormbBySesid()
      {
		  
          $sql = "SELECT e.*," 
		  . "\n c.price" 
		  . "\n FROM " . self::dTable . " as e" 
		  . "\n LEFT JOIN " . self::cTable . " as c ON c.form_id = e.form_id" 
		  . "\n WHERE c.sesid ='" . Registry::get("Users")->sesid . "'"
		  . "\n AND e.sesid ='" . Registry::get("Users")->sesid . "'";
      ...
      public function processEstimator()
      {
          ...
          if ($_POST['is_service'] == 1 and !isset($_POST['gateways']))
              Filter::$msgs['gateways'] = Lang::$word->ESTM_GATESEL_R;
		  ...
		  if (empty(Filter::$msgs)) {
			  $data = array(
					  ...
					  'showhours' => intval($_POST['showhours']),
					  'dpd' => (float)$_POST['dpd'],
					  'description' => sanitize($_POST['description']),
					  'sendmessage' => sanitize($_POST['sendmessage']),
					  'submit_btn' => sanitize($_POST['submit_btn']),
					  'is_service' => intval($_POST['is_service']),
					  'gateways' => isset($_POST['gateways']) ? Core::_implode($_POST['gateways']) : 0,
			  );
			  if (!Filter::$id) {
					$data['created'] = "NOW()";
			  }	
      ...
      /**
       * Estimator::getFormSubmittedData()
       * 
       * @return
       */
      public function getFormSubmittedData()
      {
		  ...
		  $pager = Paginator::instance();
		  $pager->items_total = countEntries(self::dTable);
		  $pager->default_ipp = Registry::get("Core")->perpage;
		  $pager->paginate();
      ...
      /**
       * Estimator::saveEstimatorData()
       * 
	   * @param bool $form_id
       * @return
       */
	  public function saveEstimatorData($form_id = false)
	  {
		  $id = ($form_id) ? $form_id : Filter::$id;
		  ...
		  switch ($type) {
			  case "radio":
				  ...
				  <div class="two fields">
					...
					<div class="field">
					  <div class="push-right">
						<div class="wojo small icon buttons"> <a class="wojo positive button" id="btnrAdd"><i class="icon add"></i></a> <a class="wojo negative button" id="btnDel"><i class="icon minus"></i></a> </div>
					  </div>
					</div>
				  </div>
				  ...
				  <div class="two fields">
					...
					<div class="field">
					  <div class="two fields">
						<div class="field">
						  <label>' . Lang::$word->ESTM_F_IPRICE . '</label>
						</div>
						<div class="field">
			  ...
			  case "check":
				  ...
				  <div class="two fields">
					...
					<div class="field">
					  <div class="push-right">
						<div class="wojo small icon buttons"> <a class="wojo positive button" id="btncAdd"><i class="icon add"></i></a> <a class="wojo negative button" id="btnDel"><i class="icon minus"></i></a> </div>
					  </div>
					</div>
				  </div>
				  <div class="wojo thin top attached divider"></div>
				  ...
				  <div class="field">
					<div class="inline-group">
					  <label class="input"> <i class="icon-append icon check"></i>
			  ...
			  case "selbox":
				  ...
				  <div class="two fields temps">
					...
					<div class="field">
					  <div class="push-right">
						<div class="wojo small icon buttons"> <a class="wojo positive button" id="btnsAdd"><i class="icon add"></i></a> <a class="wojo negative button" id="btnDel"><i class="icon minus"></i></a> </div>
					  </div>
					</div>
				  </div>
				  <div class="wojo thin top attached divider"></div>
          ...
          switch ($row->type) {
              case "radio":
					...
					foreach ($radio_arr as $i => $rselect) {
						...
						$html .= '
						<div id="container1" class="two fields clonedInput' . $k . '">
						  <div class="field">
						  ...
						  <div class="field">
							<label class="input"> <i class="icon-prepend icon dollar"></i>
							  <input type="text" value="' . (!empty($price_arr[$i]) ? $price_arr[$i] : '0.00') . '" name="price[]">
							</label>
						  </div>
						  <div class="field">
							<label class="radio">
              ...
              case "selbox":
					...
					<div class="two fields temps">
					  ...
					  <div class="field">
						<div class="push-right">
						  <div class="wojo small icon buttons"> <a id="btnsAdd" class="wojo positive button"><i class="icon add"></i></a> 
						  <a id="btnDel" class="wojo negative button"><i class="icon minus"></i></a> </div>
						</div>
					  </div>
					</div>
      ...
      /**
       * Estimator::processEstimatorField()
       * 
       * @return
       */
	  public function processEstimatorField()
	  {
					  ...
					  if (Filter::$id) {
						  ...
						  if (isset($_POST[$rows->name])) {
							  if (is_array($_POST[$rows->name])) {
								  ...
								  $htmldata['price'] = self::_implodeFields($_POST['price']);
							  }
						  } else {
							  $htmldata['attr'] = "NULL";
							  $htmldata['price'] = "NULL";
						  ...
						  if (isset($_POST['radio'])) {
							  if (is_array($_POST['radio'])) {
								  ...
								  $data['price'] = self::_implodeFields($_POST['price']);
							  }
						  } else {
							  $data['attr'] = "NULL";
							  $data['price'] = "NULL";
          ...
          if (is_object($row)) {
			  ...
			  if (count($radiodata) > 1) {
				  ...
				  foreach ($radiodata as $i => $rdata) {
					  $k = $i + 1;
					  $price = ' <span class="wojo label ' . ((!empty($price_arr[$i]) and $price_arr[$i] !=0) ? 'negative">(' . Registry::get("Core")->formatMoney($price_arr[$i]) : 'positive">(0.00') . ')</span>';
					  $sel = ($k == $row->defval) ? ' checked = "checked"' : null;
					  $html .= '<label class="radio">';
					  $html .= '<input type="radio" value="' . $rdata . '" data-price="' . $price_arr[$i] . '" name="' . $row->name . '"' . $sel . ' />';
					  $html .= '<i></i>' . $rdata . $price . '</label>';
				  }
				  unset($rdata);
			  }
              $html .= '</div>' . "\n";
          ...
          if (is_object($row)) {
			  ...
			  if (count($checkdata) >= 1) {
				  ...
				  foreach ($checkdata as $i => $cdata) {
					  ...
					  $html .= '<input type="checkbox" value="' . $cdata . '" data-price="' . $price_arr[$i] . '" name="' . $row->name . '[]"' . $sel . ' />';
					  $html .= '<i></i>' . $cdata . $price . '</label>';
				  }
				  unset($rdata);
			  }
              $html .= '</div>' . "\n";
	  ...
	  {
          $row = self::$db->fetch_all("SELECT * FROM gateways WHERE id IN($array)");
		  return ($row) ? $row : 0;
	  }
	  
	  /**
	   * Estimator::_implodeFields()
	   * 
	   * @param mixed $array
	   * @return
	   */
	  private static function _implodeFields($array)
	  {
          if (is_array($array)) {
			  $result = array();
      ...
      /**
       * Estimator::processEstimatorFieldTemp()
       * 
	   * @param array $row
       * @return
       */
	  public function processEstimatorFieldTemp($row)
	  {
